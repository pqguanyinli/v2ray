...
get_shadowsocks_config_qr_link() {
	if [[ $shadowsocks ]]; then
		...
		local link=$(curl -s --upload-file /tmp/233blog_shadowsocks.png "https://transfer.sh/${random}_233now_shadowsocks.png")
		if [[ $link ]]; then
			echo
...
get_v2ray_config_link() {
	...
	echo
	local random=$(echo $RANDOM-$RANDOM-$RANDOM | base64 -w 0)
	local link=$(curl -s --upload-file $v2ray_client_config "https://transfer.sh/${random}_233now_v2ray.json")
	if [[ $link ]]; then
		echo
...
create_v2ray_config_text() {
	...
	if [[ $v2ray_transport == [45] ]]; then
		if [[ ! $caddy ]]; then
			echo
			echo " 警告！请自行配置 TLS...教程: https://233now.com/post/3/"
		fi
		echo
...
get_v2ray_config_info_link() {
	...
	local random=$(echo $RANDOM-$RANDOM-$RANDOM | base64 -w 0)
	local link=$(curl -s --upload-file /tmp/233blog_v2ray.txt "https://transfer.sh/${random}_233now_v2ray.txt")
	if [[ $link ]]; then
		echo
		echo "---------- V2Ray 配置信息链接-------------"
		echo
		echo -e "$yellow 链接 = $cyan$link$none"
		echo
		echo -e " V2Ray 客户端使用教程: https://233now.com/post/4/"
		echo
		echo "备注...链接将在 14 天后失效..."
		echo
		echo "提醒...请不要把链接分享出去...除非你有特别的理由...."
...
get_v2ray_config_qr_link() {
	...
	local random=$(echo $RANDOM-$RANDOM-$RANDOM | base64 -w 0)
	local link=$(curl -s --upload-file /tmp/233blog_v2ray.png "https://transfer.sh/${random}_233now_v2ray.png")
	if [[ $link ]]; then
		echo
		echo "---------- V2Ray 二维码链接 -------------"
		echo
...
update_v2ray.sh() {
	if [[ $_test ]]; then
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/pqguanyinli/v2ray/test/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	else
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/pqguanyinli/v2ray/master/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	fi
...
_help() {
	echo
	echo "........... V2Ray 管理脚本帮助信息 by 233now.com .........."
	echo -e "

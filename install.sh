...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		if [[ ! -d $(pwd)/config ]]; then
			...
			echo
			echo -e " 请确保你有完整的上传 233now.com 的 V2Ray 一键安装脚本 & 管理脚本到当前 ${green}$(pwd) $none目录下"
			echo
			exit 1
		fi
		...
		if [[ $_test ]]; then
			git clone https://github.com/pqguanyinli/v2ray -b test /etc/v2ray/233boy/v2ray
		else
			git clone https://github.com/pqguanyinli/v2ray /etc/v2ray/233boy/v2ray
		fi
...
uninstall() {
	...
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
		...
		备注...仅支持卸载使用我 (233now.com) 提供的 V2Ray 一键安装脚本
		" && exit 1
	fi
...
while :; do
	echo
	echo "........... V2Ray 一键安装脚本 & 管理脚本 by 233now.com .........."
	echo
	echo "帮助说明: https://233now.com/post/1/"
	echo
	echo "搭建教程: https://233now.com/post/2/"
	echo
	echo " 1. 安装"
	echo
	echo " 2. 卸载"
